// src/Components/ModeratorDisplay/ModeratorDisplay.jsx
import React, {
  useCallback,
  useEffect,
  useMemo,
...
import ModeratorUploadFolder from '../ModeratorUploadFolder/ModeratorUploadFolder';
import ModeratorCreateFolder from '../ModeratorCreateFolder/ModeratorCreateFolder';
import ModeratorUploadFile from '../ModeratorUploadFile/ModeratorUploadFile';
...
function ModeratorDisplay() {
  ...
  // Stream endpoint for videos (supports HTTP Range/seek)
  ...
  const handlePreview = async (item) => {
    ...
    try {
      ...
      // VIDEO — use streaming URL (no blob) to enable seeking and quick start
      else if (isVid(item.name)) {
        const streamUrl = `${API_BASE}/api/stream?path=${encodeURIComponent(
          filePath
        )}`;
        setPreviewData({
          type: 'video',
      ...
      <div className="libd-toolbar">
        ...
        <div className="libd-toolbar-row libd-toolbar-actions">
          <div className="libd-actions">
            ...
            />
            <ModeratorUploadFolder
              currentPath={path}
              onFolderUploaded={handleFolderUploaded}
            />
            <ModeratorCreateFolder
              currentFolder={{
                fullPath: path,
