// src/Components/FinanceUploadFile/FinanceUploadFile.jsx
...
const RAW_FIN_ROOT = process.env.REACT_APP_FINANCE_START_PATH || '/finance';
const FIN_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/finance';
})(RAW_FIN_ROOT);
...
/* ================= Component ================= */
const FinanceUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToFinance(currentPath || FIN_ROOT);
...
FinanceUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/finance", "/finance/Docs/2024") */
