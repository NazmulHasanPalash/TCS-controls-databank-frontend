// src/Components/NewProductionUploadFile/NewProductionUploadFile.jsx
...
import './NewProductionUploadFile.css';
...
const RAW_NEW_PRODUCTION_ROOT =
  process.env.REACT_APP_NEW_PRODUCTION_START_PATH || '/new-production';
const NEW_PRODUCTION_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/new-production';
})(RAW_NEW_PRODUCTION_ROOT);
...
const clampToNewProduction = (dest) => {
  const root = NEW_PRODUCTION_ROOT;
...
/* ================= Component ================= */
const NewProductionUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToNewProduction(currentPath || NEW_PRODUCTION_ROOT);
...
NewProductionUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/new-production", "/new-production/Docs/2024") */
...
export default NewProductionUploadFile;
