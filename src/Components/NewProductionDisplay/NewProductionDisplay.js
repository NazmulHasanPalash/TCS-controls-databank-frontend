// src/Components/NewProductionDisplay/NewProductionDisplay.jsx
import React, {
  useCallback,
  useEffect,
  useMemo,
...
import './NewProductionDisplay.css';

import BredCrum from '../BreadCrum/BreadCrum';
import NewProductionUploadFolder from '../NewProductionUploadFolder/NewProductionUploadFolder';
import NewProductionCreateFolder from '../NewProductionCreateFolder/NewProductionCreateFolder';
import NewProductionUploadFile from '../NewProductionUploadFile/NewProductionUploadFile';
...
const START_PATH =
  (process.env.REACT_APP_PRODUCTION_START_PATH || '/new-production').replace(
    /\/+$/,
    ''
  ) || '/new-production';
...
function NewProductionDisplay() {
  ...
  const handleDownload = (item) => {
    if (item.isDirectory) {
      // Extra safety, though we don't show Download for folders anymore
        ...
        <div className="libd-toolbar">
          ...
          <div className="libd-toolbar-row libd-toolbar-actions">
            <div className="libd-actions">
              <NewProductionUploadFile
                currentPath={path}
                onFileUploaded={fetchList}
              />
              <NewProductionUploadFolder
                currentPath={path}
                onFolderUploaded={handleFolderUploaded}
              />
              <NewProductionCreateFolder
                currentFolder={{
                  fullPath: path,
                  name: path.split('/').filter(Boolean).pop() || 'production',
                }}
                onNewProductionCreateFolder={fetchList}
...
NewProductionDisplay.propTypes = {};

export default NewProductionDisplay;
