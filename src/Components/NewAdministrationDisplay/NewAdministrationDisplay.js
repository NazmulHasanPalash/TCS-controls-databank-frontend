...
import './NewAdministrationDisplay.css';

import BredCrum from '../BreadCrum/BreadCrum';
import NewAdministrationUploadFolder from '../NewAdministrationUploadFolder/NewAdministrationUploadFolder';
import NewAdministrationCreateFolder from '../NewAdministrationCreateFolder/NewAdministrationCreateFolder';
import NewAdministrationUploadFile from '../NewAdministrationUploadFile/NewAdministrationUploadFile';
...
const ADMIN_START_PATH =
  (process.env.REACT_APP_ADMIN_START_PATH || '/new-administration').replace(
    /\/+$/,
    ''
  ) || '/new-administration';
...
const clampToBase = (targetPath) => {
  const base = normalizePath(ADMIN_START_PATH);
...
/* ================= Component ================= */
function NewAdministrationDisplay() {
  const [path, setPath] = useState(() => normalizePath(ADMIN_START_PATH));
      ...
      <div className="libd-toolbar">
        ...
        <div className="libd-toolbar-row">
          <BredCrum
            ...
            path={path}
            onNavigate={(p) => {
              const target = p === '/' ? ADMIN_START_PATH : p;
        ...
        <div className="libd-toolbar-row libd-toolbar-actions">
          <div className="libd-actions">
            <NewAdministrationUploadFile
              currentPath={path}
              onFileUploaded={fetchList}
            />
            <NewAdministrationUploadFolder
              currentPath={path}
              onFolderUploaded={handleFolderUploaded}
            />
            <NewAdministrationCreateFolder
              currentFolder={{
                fullPath: path,
                name:
                  path.split('/').filter(Boolean).pop() || 'new-administration',
              }}
              onNewAdministrationCreateFolder={fetchList}
...
NewAdministrationDisplay.propTypes = {};

export default NewAdministrationDisplay;
