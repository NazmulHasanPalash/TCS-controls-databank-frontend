// src/Components/NewAdministrationUploadFile/NewAdministrationUploadFile.jsx
...
import './NewAdministrationUploadFile.css';
...
const RAW_ADMIN_ROOT =
  process.env.REACT_APP_ADMIN_START_PATH || '/new-administration';
const ADMIN_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/new-administration';
})(RAW_ADMIN_ROOT);
...
/* ================= Component ================= */
const NewAdministrationUploadFile = ({
  currentPath,
  userId,
  onFileUploaded,
}) => {
  ...
  const destPath = clampToAdmin(currentPath || ADMIN_ROOT);
...
NewAdministrationUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/new-administration", "/new-administration/Docs/2024") */
...
export default NewAdministrationUploadFile;
