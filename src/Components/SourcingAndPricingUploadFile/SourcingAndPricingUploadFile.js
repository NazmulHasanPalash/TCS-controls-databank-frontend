// src/Components/SourcingAndPricingUploadFile/SourcingAndPricingUploadFile.jsx
...
/* ================= Component ================= */
const SourcingAndPricingUploadFile = ({
  currentPath,
  userId,
  onFileUploaded,
}) => {
  ...
  // Destination follows the EXACT folder the user is viewing, clamped under /sourcing-pricing
  const destPath = clampToRoot(currentPath || SNP_ROOT);
  ...
  const handleFileSubmit = async (e) => {
    ...
    fd.append('path', destPath); // backend saves into this exact folder
    fd.append('file', file); // field name must match backend
        ...
        <Modal.Body>
          ...
          {isUploading ? (
            ...
            <Form onSubmit={handleFileSubmit} encType="multipart/form-data">
              ...
              <Form.Group controlId="formSnpFile" className="my-2">
                <Form.Control
                  type="file"
...
SourcingAndPricingUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/sourcing-pricing", "/sourcing-pricing/2025/Q1") */
...
export default SourcingAndPricingUploadFile;
