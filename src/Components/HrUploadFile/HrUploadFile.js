// src/Components/HrUploadFile/HrUploadFile.jsx
...
const RAW_HR_ROOT = process.env.REACT_APP_HR_START_PATH || '/hr';
const HR_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/hr';
})(RAW_HR_ROOT);
...
/* ================= Component ================= */
const HrUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToHr(currentPath || HR_ROOT);
...
HrUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/hr", "/hr/Policies/2024") */
