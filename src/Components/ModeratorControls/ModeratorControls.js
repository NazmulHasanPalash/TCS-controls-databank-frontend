// src/components/ModeratorControls.jsx
// Moderator UI to set a user's role (except admin), list all users, and DELETE users by UID.
...
// Must match server ALLOWED_ROLES, but moderators CANNOT assign "admin":
// [
//   'new_register',
//   'user',
//   'new_sales',
...
//   'admin',
// ]
// For moderators we EXCLUDE "admin" from assignable roles.
const ROLES = [
  // Base
  'new_register',
  'user',
...
export default function ModeratorControls() {
  ...
  // Safer default than elevated roles
  ...
  async function setRoleByEmail(payload) {
    // Backend endpoint is the same; server should also enforce no admin from moderator
  ...
  const save = async (e, opts = { reset: false }) => {
    ...
    // Explicitly block "admin" role even if someone tampers with the DOM.
    if (role === 'admin') {
      flash('Moderators are not allowed to assign the admin role.', 'danger');
      return;
    }
    ...
    <div className="ac-root ">
      <div className="ac-shell">
        <header className="ac-header">
          <div>
            <h3 className="ac-title">Moderator — Dashboard</h3>
            <p className="ac-subtitle">
              Manage user roles (except admin) and access in a single, clean
              view.
            </p>
          </div>
        ...
        <section className="ac-card ac-card--accent">
          ...
          >
            <div className="ac-field ac-field--full">
              <label className="ac-label" htmlFor="mc-email">
                Email
              </label>
              <input
                id="mc-email"
              ...
              <small className="ac-help">
                Enter the user&apos;s email and choose a role (except admin),
                then click Save.
              </small>
            </div>

            <div className="ac-field">
              <label className="ac-label" htmlFor="mc-name">
                Name
              </label>
              <input
                id="mc-name"
            ...
            <div className="ac-field">
              <label className="ac-label" htmlFor="mc-role">
                Role
              </label>
              <select
                id="mc-role"
