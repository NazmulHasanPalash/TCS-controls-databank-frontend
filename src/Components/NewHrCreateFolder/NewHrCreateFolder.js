...
import './NewHrCreateFolder.css';

/* ======================== Config ======================== */
const RAW_API_BASE =
  process.env.REACT_APP_API_BASE || 'https://databank.tcscontrols.com.my';

// Base path for New HR section
const NEW_HR_BASE = process.env.REACT_APP_NEW_HR_BASE || '/new-hr';
...
function resolveParentPath(currentFolder) {
  ...
  const parts = [NEW_HR_BASE, ...names, leafName].filter(Boolean);
...
NewHrCreateFolder.propTypes = {
  currentFolder: PropTypes.oneOfType([PropTypes.object, PropTypes.string]),
  onNewHrCreateFolder: PropTypes.func, // (meta) => void
...
NewHrCreateFolder.defaultProps = {
  currentFolder: 'root folder',
  onNewHrCreateFolder: undefined,
  siblings: undefined,
...
export default NewHrCreateFolder;
