// src/Components/NewHrUploadFile/NewHrUploadFile.jsx
...
const RAW_NEW_HR_ROOT =
  process.env.REACT_APP_NEW_HR_BASE ||
  process.env.REACT_APP_HR_BASE ||
  '/new-hr';

const NEW_HR_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/new-hr';
})(RAW_NEW_HR_ROOT);
...
/* ================= Component ================= */
const NewHrUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToNewHr(currentPath || NEW_HR_ROOT);
...
NewHrUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/new-hr", "/new-hr/Docs/2024") */
...
export default NewHrUploadFile;
