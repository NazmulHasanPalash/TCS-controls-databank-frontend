// src/Components/NewSalesUploadFile/NewSalesUploadFile.jsx
...
// Root for New Sales
const RAW_NEW_SALES_ROOT = process.env.REACT_APP_NEW_SALES_BASE || '/new-sales';
const NEW_SALES_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/new-sales';
})(RAW_NEW_SALES_ROOT);
...
const clampToNewSales = (dest) => {
  const root = NEW_SALES_ROOT;
...
/* ================= Component ================= */
const NewSalesUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToNewSales(currentPath || NEW_SALES_ROOT);
...
NewSalesUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/new-sales", "/new-sales/Docs/2024") */
...
export default NewSalesUploadFile;
