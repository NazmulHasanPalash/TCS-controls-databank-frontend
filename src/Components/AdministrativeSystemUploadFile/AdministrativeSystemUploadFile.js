// src/Components/AdministrativeSystemUploadFile/AdministrativeSystemUploadFile.jsx
...
import './AdministrativeSystemUploadFile.css';
...
const ADMIN_SYSTEM_ROOT = (function normRoot(r) {
  const t =
    ...
    String(r || '')
      ...
      .replace(/^\/+/, '');
  return (
    t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/administrative-system'
  );
})(RAW_ADMIN_SYSTEM_ROOT);
...
const clampToAdministrativeSystem = (dest) => {
  const root = ADMIN_SYSTEM_ROOT;
...
/* ================= Component ================= */
const AdministrativeSystemUploadFile = ({
  currentPath,
  userId,
  onFileUploaded,
}) => {
  ...
  const destPath = clampToAdministrativeSystem(
    currentPath || ADMIN_SYSTEM_ROOT
  );
...
AdministrativeSystemUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/administrative-system", "/administrative-system/Docs/2024") */
...
export default AdministrativeSystemUploadFile;
