// src/Components/NewFinanceUploadFile/NewFinanceUploadFile.jsx
...
import './NewFinanceUploadFile.css';
...
// Base for New Finance area
const RAW_NEW_FINANCE_ROOT =
  process.env.REACT_APP_NEW_FINANCE_BASE || '/new-finance';
const NEW_FINANCE_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/new-finance';
})(RAW_NEW_FINANCE_ROOT);
...
const clampToNewFinance = (dest) => {
  const root = NEW_FINANCE_ROOT;
...
/* ================= Component ================= */
const NewFinanceUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToNewFinance(currentPath || NEW_FINANCE_ROOT);
...
NewFinanceUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/new-finance", "/new-finance/Docs/2024") */
...
export default NewFinanceUploadFile;
