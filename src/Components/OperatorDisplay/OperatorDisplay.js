...
const START_PATH =
  (process.env.REACT_APP_OPERATOR_START_PATH || '/operator').replace(
    /\/+$/,
    ''
  ) || '/operator';
...
function OperatorDisplay() {
  ...
  // Prefer streaming (supports HTTP Range & seeking) for video preview
  ...
  const handlePreview = async (item) => {
    ...
    try {
      ...
      // VIDEO — use streaming URL (no blob) to enable seeking and instant play
