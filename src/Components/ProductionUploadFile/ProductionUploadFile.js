// src/Components/ProductionUploadFile/ProductionUploadFile.jsx
...
import './ProductionUploadFile.css';
...
// Root for Production area (you can change env name / default as needed)
const RAW_PROD_ROOT =
  process.env.REACT_APP_PRODUCTION_START_PATH || '/production';

const PROD_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/production';
})(RAW_PROD_ROOT);
...
/* ================= Component ================= */
const ProductionUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToProduction(currentPath || PROD_ROOT);
...
ProductionUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/production", "/production/Docs/2024") */
...
export default ProductionUploadFile;
