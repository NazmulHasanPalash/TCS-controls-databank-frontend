...
import HrUploadFolder from '../HrUploadFolder/HrUploadFolder';
import HrCreateFolder from '../HrCreateFolder/HrCreateFolder';
import HrUploadFile from '../HrUploadFile/HrUploadFile';
...
const START_PATH =
  (process.env.REACT_APP_HR_START_PATH || '/hr').replace(/\/+$/, '') || '/hr';
...
function HrDisplay() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      setFolderSizes({}); // reset cache on path change
    } catch (err) {
      ...
      <div className="libd-toolbar">
        ...
        <div className="libd-toolbar-row libd-toolbar-actions">
          <div className="libd-actions">
            <HrUploadFile currentPath={path} onFileUploaded={fetchList} />
            <HrUploadFolder
              currentPath={path}
            ...
            <HrCreateFolder
              currentFolder={{
                fullPath: path,
                name: path.split('/').filter(Boolean).pop() || 'hr',
              }}
              onHrCreateFolder={fetchList}
