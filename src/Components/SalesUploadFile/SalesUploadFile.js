// src/Components/SalesUploadFile/SalesUploadFile.jsx
...
const RAW_SALES_ROOT = process.env.REACT_APP_SALES_START_PATH || '/sales';
const SALES_ROOT = (function normRoot(r) {
  const t =
    '/' +
    String(r || '')
      .replace(/\\/g, '/')
      .replace(/^\/+/, '');
  return t.replace(/\/{2,}/g, '/').replace(/\/+$/, '') || '/sales';
})(RAW_SALES_ROOT);
...
const clampToSales = (dest) => {
  const root = SALES_ROOT;
...
/* ================= Component ================= */
const SalesUploadFile = ({ currentPath, userId, onFileUploaded }) => {
  ...
  const destPath = clampToSales(currentPath || SALES_ROOT);
...
SalesUploadFile.propTypes = {
  /** The exact path the user is viewing (e.g. "/sales", "/sales/Docs/2024") */
...
export default SalesUploadFile;
